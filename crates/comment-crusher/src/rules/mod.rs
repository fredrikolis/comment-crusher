// Concern: the rule set — its thresholds, and running every rule a scanned file is subject to | Non-concern: what a rule measures, or a file that was never scanned | IO: (Scan) -> Vec<Diagnostic>
